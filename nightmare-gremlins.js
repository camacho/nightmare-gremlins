...
export function injectGremlins() {
  return function executeInjectGremlins(nightmare) {
    nightmare.inject(
      'js',
      path.join(__dirname, 'node_modules/gremlin-monkeys/gremlins.min.js'),
    );
  };
}

export function createHorde(_options = {}) {
  const options = Object.assign({}, defaults.create, _options);

  return function executeCreateHorde(nightmare) {
    nightmare.evaluate((jsonOptions) => {
      window.horde = window.gremlins.createHorde(JSON.parse(jsonOptions));
